...
public class AdviceActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<ArrayList<adviceModel>> call = retrofitInterface.executeGetAdvices(map);
            ...
            public void onResponse(Call<ArrayList<adviceModel>> call, Response<ArrayList<adviceModel>> response) {
                if(response.code() == 200){
                    for (int i = 0; i < response.body().size(); i++) {
                        listLogs.add(response.body().get(i).getText());
                    }

                    ListView my_listview = (ListView)findViewById(R.id.listView1);
                    ArrayAdapter<String> adapter = new ArrayAdapter<String>(AdviceActivity.this, android.R.layout.simple_list_item_1, listLogs);
